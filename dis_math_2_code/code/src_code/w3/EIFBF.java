...
//    }
    ...
    Set<Integer> setVertices = new HashSet<>();
    for (Vertex i : vertexes) {
      int nMales = 0;
      int nFemales = 0;
      set = new HashSet<>();
...
//        }
        ...
        vertexes[max].nMales = nMales;
        vertexes[max].nFemales = nFemales;
        setVertices.add(max);
      }
    }

    List<Integer> list = new ArrayList<>(setVertices);
    Collections.sort(list);
    for (Integer i : list) {
      sb.append(vertexes[i].id + 1).append(" ").append(vertexes[i].nMales).append(" ").append(vertexes[i].nFemales).append("\n");
    }
