...
class EIMAXHTR {
  ...
  public static void findVertex() {
    ...
    // Assign level using dfs()
    dfs(vertexList.get(0));

    // Find the vertex with the highest level
    Vertex highestVertex = vertexList.get(0);
    int maxLevel = 0;
    ...
    // Re-create vertices list
    for (Vertex v : vertexList) {
      v.isVisited = false;
      v.level = 0;
    }

    // Depth-first-search with the highest level
    dfs(highestVertex);
    int id = highestVertex.id;
    maxLevel = -1;
    for (Vertex v : vertexList) {
      if (maxLevel < v.level) {
        maxLevel = v.level;
    ...
    if(id > highestVertex.id) {
      id = highestVertex.id;
    }
    System.out.println(id + " " + maxLevel);
